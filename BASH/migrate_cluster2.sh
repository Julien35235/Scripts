...
# Liste des IDs à exclure, séparés par des barres verticales |
EXCLUDE_VMIDS="^(103|108|122)$"
...
pvesh get /cluster/resources --type vm --output-format json | tr -d '\n' | sed 's/},{/\n/g' | while read -r line; do
    ...
    # --- CORRECTION DE LA CONDITION D'EXCLUSION ---
    if [[ "$vmid" =~ $EXCLUDE_VMIDS ]]; then
        echo -e "\n${BLUE}[ℹ] VM ID: $vmid détectée dans la liste d'exclusion (PBS/Autre). Ignorée.${NC}"
        continue
    fi
    # -----------------------------------------------
