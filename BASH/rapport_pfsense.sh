...
# =========================================================
# Rapport Système Firewall (OPNsense / pfSense)
# Auteur : Julien Despagne
# Version : FreeBSD Compatible
# Description :
# Génère un rapport HTML complet compatible :
# - OPNsense
# - pfSense
# =========================================================

OUTPUT="/root/rapport_firewall.html"
...
[ -z "$OPENVPN" ] && \
OPENVPN="Aucun processus OpenVPN détecté"
...
[ -z "$IPSEC" ] && \
IPSEC="Aucun tunnel IPSec détecté"
...
if command -v wg >/dev/null 2>&1; then
    ...
    WIREGUARD=$(ifconfig | awk '/^wg/ {print $1}' | while read i
    do
        echo "========== $i =========="
        ifconfig "$i"
        echo
...
[ -z "$WIREGUARD" ] && \
WIREGUARD="Aucun tunnel WireGuard détecté"
...
[ -z "$CERTS" ] && \
CERTS="Aucun certificat détecté"
...
if command -v pfSense-upgrade >/dev/null 2>&1; then
    ...
    UPDATES=$(pfSense-upgrade -d 2>/dev/null)
...
[ -z "$TEMP" ] && \
TEMP="Température non disponible"
